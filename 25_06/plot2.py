...
class VicsekModel(object):
    ...
    def __init__(self, L=7, eta=2.0, N=300, num_steps=10, r=1.0, dt=1, v=0.01):
    ...
    def main(self, visual_mode):
        ...
        if visual_mode:
            ...
            for step in xrange(self.num_steps):
                ...
                self.perform_step()
                self.plot_grid(axes);
                plt.savefig('new/picture'+str(step+1))
                plt.gcf().clear()

            print "for eta =", self.eta, " v_a =", self.find_avg_norm_vel() ," n_o =" , self.nematic_order()
        ...
        # otherwise we are in data intensive mode
            ...
            for self.eta in arange(0.0, 4.0, 0.2):
                ...
                print self.eta, "\t", self.find_avg_norm_vel(), "\t" ,self.nematic_order()
    ...
    def perform_step(self):
        ...
        for p in xrange(self.N):
            ...
            near_angles = np.array(self.angles[self.particles_in_range[p, :]])
            diff_angles = np.array([np.sign(np.cos(x-self.angles[p])) for x in near_angles])



            # and average over these
            mean_directions[p] = np.arctan2(np.mean(diff_angles*np.sin(near_angles)),np.mean(diff_angles*np.cos(near_angles)))
    ...
    def nematic_order(self):
        ...
        sum_cos = np.sum(np.cos(2*self.angles))
        sum_sin = np.sum(np.sin(2*self.angles))
        sum_sqr = np.power((sum_cos**2+sum_sin**2),0.5)

        return sum_sqr/self.N
...
sim = VicsekModel(L, eta, N, num_steps,v=0.3)
